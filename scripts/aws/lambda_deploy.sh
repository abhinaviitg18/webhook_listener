...
lambda_update_configuration_with_retry() {
  ...
  for ((i=1; i<=attempts; i++)); do
    if output="$(aws lambda update-function-configuration \
      ...
      --environment "Variables={APP_ENV_SSM_PARAM=$APP_ENV_SSM_PARAM,APP_ENV_INLINE_B64=$APP_ENV_INLINE_B64,LAMBDA_ORIGIN_SHARED_SECRET=$LAMBDA_ORIGIN_SHARED_SECRET,APP_RUNTIME_ENV=$LAMBDA_ENVIRONMENT,LANGFUSE_ENABLED=$LANGFUSE_ENABLED,LANGFUSE_HOST=$LANGFUSE_HOST,LANGFUSE_PUBLIC_KEY=$LANGFUSE_PUBLIC_KEY,LANGFUSE_SECRET_KEY=$LANGFUSE_SECRET_KEY}" 2>&1)"; then
      return 0
    fi
